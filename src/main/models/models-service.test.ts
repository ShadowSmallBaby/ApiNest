import { ModelsService } from './models-service';
import type { AccountEntity } from '../storage/repositories/account-repository';
import type { ModelRecord } from '../../shared/ipc/bridge';

const ACCOUNT_ID = '00000000-0000-4000-8000-000000000001';
const SITE_USER_ID = '42';

const withSiteUserId = { getSiteUserId: () => SITE_USER_ID };

function makeAccount(overrides: Partial<AccountEntity> = {}): AccountEntity {
  return {
    id: ACCOUNT_ID,
    siteId: 'site-1',
    siteName: 'Demo',
    platform: 'newapi',
    baseUrl: 'https://demo.example.com',
    displayName: 'Main',
    routeProfile: 'modern',
    authRefId: null,
    recordVersion: 1,
    createdAt: '2026-01-01T00:00:00.000Z',
    updatedAt: '2026-01-01T00:00:00.000Z',
    ...overrides,
  };
}

const SAMPLE_MODEL: ModelRecord = {
  modelName: 'gpt-4o',
  quotaType: 0,
  modelRatio: 2.5,
  completionRatio: 3,
  modelPrice: 0,
  enableGroups: ['default'],
  supportedEndpointTypes: ['chat/completions'],
  availableForAccount: true,
};

describe('ModelsService', () => {
  it('resolves the account base url and delegates to the models client', async () => {
    let capturedRequest: { accountId: string; baseUrl: string; siteUserId: string } | null = null;
    const service = new ModelsService({
      accountRepository: { get: () => makeAccount() },
      authStateRepository: withSiteUserId,
      modelsClient: {
        listByAccount: async request => {
          capturedRequest = request;
          return [SAMPLE_MODEL];
        },
      },
    });

    const result = await service.listByAccount(ACCOUNT_ID);
    expect(result).toEqual([SAMPLE_MODEL]);
    expect(capturedRequest).toEqual({
      accountId: ACCOUNT_ID,
      baseUrl: 'https://demo.example.com',
      siteUserId: SITE_USER_ID,
    });
  });

  it('throws ACCOUNT_NOT_FOUND for an unknown account', async () => {
    const service = new ModelsService({
      accountRepository: { get: () => null },
      authStateRepository: withSiteUserId,
      modelsClient: { listByAccount: async () => [] },
    });

    await expect(service.listByAccount(ACCOUNT_ID)).rejects.toMatchObject({ code: 'ACCOUNT_NOT_FOUND' });
  });

  it('rejects non-newapi platforms with NOT_IMPLEMENTED', async () => {
    const service = new ModelsService({
      accountRepository: { get: () => makeAccount({ platform: 'sub2api' }) },
      authStateRepository: withSiteUserId,
      modelsClient: { listByAccount: async () => [] },
    });

    await expect(service.listByAccount(ACCOUNT_ID)).rejects.toMatchObject({ code: 'NOT_IMPLEMENTED' });
  });

  it('throws AUTH_METADATA_REQUIRED when site user id is missing', async () => {
    let called = false;
    const service = new ModelsService({
      accountRepository: { get: () => makeAccount() },
      authStateRepository: { getSiteUserId: () => null },
      modelsClient: {
        listByAccount: async () => {
          called = true;
          return [];
        },
      },
    });

    await expect(service.listByAccount(ACCOUNT_ID)).rejects.toMatchObject({ code: 'AUTH_METADATA_REQUIRED' });
    expect(called).toBe(false);
  });
});
